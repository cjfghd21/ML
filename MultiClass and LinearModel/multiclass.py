...
class MCTree:
    ...
    def train(self, X, Y):
        for n in self.tree.iterNodes():
            ...
            for i in range(len(leftLabels)):
                leftY = logical_or(leftY, Y == leftLabels[i])
            
            rightY = (Y == rightLabels[0])
            for i in range(len(rightLabels)):
                rightY = logical_or(rightY, Y == rightLabels[i])
            
            res = logical_or(leftY, rightY)
            thisX = X[res]
            thisY = Y[res]
    
            temp = (thisY == leftLabels[0])
            for i in range(len(leftLabels)):
                temp = logical_or(temp, thisY == leftLabels[i])
    ...
    def predict(self, X):
        ...
        while not curNode.isLeaf:
            n = curNode.getNodeInfo()
            pr = n.predict_proba(X)
            if pr[0,1] > 0.5:
