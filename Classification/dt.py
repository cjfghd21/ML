...
class DT(BinaryClassifier):
    ...
    def predict(self, X):
        ...
        branch.
        """
        #recursively call predict untill we reach leaf and return its label
        if self.isLeaf == True:
            return repr(self.label)
        #else traverse left on <0.5,  right on >=0.5
        else:
    ...
    def trainDT(self, X, Y, maxDepth, used):
        ...
        if maxDepth <= 0 or len(util.uniq(Y)) <= 1:
            ...
            self.isLeaf = True
            self.label  = util.mode(Y)    # return most frequent y.
            ...
            for d in range(D):
                ...
                leftY  = Y[X[:,d] < 0.5]    ### labels resulting from feature value < 0.5

                rightY = Y[X[:,d] >= 0.5]    ### labels resulting from feature value >=0.5
                ...
                left_points = util.mode(leftY)
                right_points = util.mode(rightY)
                error = size(nonzero([leftY != left_points])) + size(nonzero([rightY != right_points]))
            ...
            if bestFeature < 0:
                ...
                ### TODO: YOUR CODE HERE
                newXL = X[X[:, self.feature] < 0.5]  #rows where best features' label is < 0.5
                newXR = X[X[:,self.feature] >= 0.5] #rows where best features' label is >= 0.5
                
                newYL = Y[X[:, self.feature] < 0.5] #labels resulting from rows where best features' label is < 0.5
                newYR = Y[X[:, self.feature] >= 0.5] #labels resulting fomr rows where best features' label is >= 0.5

                self.left.trainDT(newXL,newYL, self.left.opts['maxDepth'], used+[self.feature])
                self.right.trainDT(newXR,newYR, self.right.opts['maxDepth'], used+[self.feature])
