...
class SoftmaxRegression:
    ...
    def cost(self, X, Y, W=None):
        ...
        ### YOUR CODE HERE ###
        exp_W_X = np.exp(W_X)
        probabilities = exp_W_X / np.sum(exp_W_X, axis=0)
        cost = -1/N *(np.multiply(indicator, np.log(probabilities)).sum())
        gradient = -1/N *((indicator - probabilities).dot(X.T))
    ...
    def train(self, X, Y):
        ...
        result = scipy.optimize.minimize(J, W.flatten(), method='L-BFGS-B', jac=True, options={'maxiter': self.opts['maxIter'], 'disp': True})
